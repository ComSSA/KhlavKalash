...
class TheMysteryBox (IRegularCommand):
    ...
    def command_mysterybox(self, user, channel, args):
        if(len(args) == 0):
            ...
            if (self.playing and  self.playerIndex == self.playerlist.find(user)[1]):
                return 'debug'
    ...
    def move(self, move, user):
        if (move > box):
            ...
            return string.split(user,'!') + ' passed the box to ' + self.playerlist.players[self.playerIndex].name + '. the box displays the number ' + str(self.box) + '!'
